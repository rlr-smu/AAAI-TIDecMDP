...
class ANNPol:
    def __init__(self, featureLength, actionCount, input_var, train_mode, agent, config):
        ...
        # Output tensor
        self.l_prob = self.initNNPolicy(drate=self.config.drate)

    # Remember to change the mode while evaluating!
    def initNNPolicy(self, drate):
        
        l_in = tf.layers.dense(inputs=self.input_var, units=self.config.numUnitsPerLayer, activation=tf.nn.relu, trainable=True, name="DenseReLu_1_Policy_Agent_"+str(self.agent))
        dropout1 = tf.layers.dropout(inputs=l_in, rate=drate, training=self.train_mode, name="Dropout_1_Policy_Agent_"+str(self.agent))

        l_hid2 = tf.layers.dense(inputs=dropout1, units=self.config.numUnitsPerLayer, activation=tf.nn.relu, trainable=True, name="DenseReLu_2_Policy_Agent_"+str(self.agent))
        dropout2 = tf.layers.dropout(inputs=l_hid2, rate=drate, training=self.train_mode, name="Dropout_2_Policy_Agent_"+str(self.agent))

        l_hid3 = tf.layers.dense(inputs=dropout2, units=self.config.numUnitsPerLayer, activation=tf.nn.relu, trainable=True, name="DenseReLu_3_Policy_Agent_"+str(self.agent))
        dropout3 = tf.layers.dropout(inputs=l_hid3, rate=drate, training=self.train_mode, name="Dropout_3_Policy_Agent_"+str(self.agent))

        l_hid4 = tf.layers.dense(inputs=dropout3, units=self.config.numUnitsPerLayer, activation=tf.nn.relu, trainable=True, name="DenseReLu_4_Policy_Agent_"+str(self.agent))
        dropout4 = tf.layers.dropout(inputs=l_hid4, rate=drate, training=self.train_mode, name="Dropout_4_Policy_Agent_"+str(self.agent))
        
        logits = tf.layers.dense(inputs=dropout4, units=self.act, name="Dense_5_Policy_Agent_"+str(self.agent))
    ...
    def get_action(self, flat_obs, sess):
        prob = self.l_prob.eval({self.input_var: flat_obs[0], self.train_mode: False}, session=sess)
